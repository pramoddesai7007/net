...
const Billing = ({ tableId, acPercentage }) => {
  ...
  useEffect(() => {
    const fetchHotelInfo = async () => {
      try {
        // Fetch all hotels
        const allHotelsResponse = await axios.get('https://back-jj8x.onrender.com/api/hotel/get-all');
        ...
        if (defaultHotelId) {
          ...
          const response = await axios.get(`https://back-jj8x.onrender.com/api/hotel/get/${defaultHotelId}`);
